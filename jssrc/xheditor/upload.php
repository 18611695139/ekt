...
$upExt='txt,jpg,png,gif,doc,docx,xls,xlsx,csv,ppt,rar,zip,pdf,wmv,avi,wma,mp3,mid,wav,swf';//上传扩展名
$msgType=2;//返回上传参数的格式：1，只返回url，2，返回参数数组
$immediate=isset($_GET['immediate'])?$_GET['immediate']:0;//立即上传模式，仅为演示用

$httpurl = "http://".$_SERVER['HTTP_HOST'].substr($_SERVER['SCRIPT_NAME'],0,-10);
$err = "";
$msg = "''";
...
if($err==''){
	...
	if(preg_match('/'.str_replace(',','|',$upExt).'/i',$extension))
	{
		...
		else
		{
			...
			else $msg="{'url':'$httpurl/".$targetPath."','localname':'".jsonString($localName)."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
		}
	}
