 ...
 * $Id    : cls_mysql.php 
 * $Author: ZT
 * $time  : Mon Aug 10 12:06:23 CST 2009
*/
...
class cls_mysql
{
	...
	function cls_mysql($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
	{
		...
		{
			/* utf-8 转为utf8*/
			$charset = strtolower(str_replace('-', '', EST_CHARSET));
		}
	...
	function query($sql, $type = '')
	{
		...
		{
			$logfilename = $this->root_path . 'log/mysql_query_' . $this->dbhash . '_' . date('Y_m_d') . '.log';
	...
	function ErrorMsg($message = '', $sql = '')
	{
		...
		{
			$logfilename = $this->root_path . 'log/error_mysql_query_' . $this->dbhash . '_' . date('Y_m_d') . '.log';
			$str = 'date: '.date('Y_m_d H:i:s')  . "\n"
			.'sql: '.$this->error_message['1']['sql'] . "\n"
			.'error: '. sprintf('%s',$this->error_message) . "\n\n";
		}
		else
		{
			$logfilename = $this->root_path . 'log/error_mysql_query_' . $this->dbhash . '_' . date('Y_m_d') . '.log';
	...
	function getAllCached($sql, $idname='',$cached = 'FILEFIRST')
	{
		$cachefirst = ($cached == 'FILEFIRST' || ($cached == 'MYSQLFIRST' && $this->platform != 'WINDOWS')) && $this->max_cache_time;
		if (!$cachefirst)
		{
			return $this->getAll($sql,$idname);
		}
		else
		{
			$result = $this->getSqlCacheData($sql, $cached);
	...
	function getColCached($sql, $idname = '', $cached = 'FILEFIRST')
	{
		$cachefirst = ($cached == 'FILEFIRST' || ($cached == 'MYSQLFIRST' && $this->platform != 'WINDOWS')) && $this->max_cache_time;
		if (!$cachefirst)
		{
			return $this->getCol($sql,$idname);
		}
		else
		{
			$result = $this->getSqlCacheData($sql, $cached);
