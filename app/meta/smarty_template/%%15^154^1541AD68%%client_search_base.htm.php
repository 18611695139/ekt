...
<form class="form-search" action="javascript:quick_search()" method="POST" name="searchForm" id="searchForm">
   ...
   <!-- <div class="btn-group" data-toggle="buttons-radio">
        <?php $_from = $this->_tpl_vars['cle_stage']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['skey'] => $this->_tpl_vars['stage']):
?>
        <button id="stage<?php echo $this->_tpl_vars['skey']; ?>
" class="btn btn-link" onclick="stage_type='<?php echo $this->_tpl_vars['stage']['name']; ?>
'; quick_search();"><?php echo $this->_tpl_vars['stage']['name']; ?>
</button>
